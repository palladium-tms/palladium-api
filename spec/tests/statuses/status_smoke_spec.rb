...
describe 'Result Smoke' do
  ...
  describe 'Create new status' do
    it 'check creating new status' do
      request = StatusFunctions.create_new_status(account)
      response = http.request(request[0])
      expect(response.code).to eq('200')
      expect(JSON.parse(response.body)['errors'].empty?).to be_truthy
      expect(JSON.parse(response.body)['status']['name']).to eq(request[1])
    ...
    it 'check creating new status with correct status_data | no name' do
      ...
      expect(response.code).to eq('200')
      expect(JSON.parse(response.body)['errors'].empty?).to be_truthy
      expect(JSON.parse(response.body)['status']['name']).to eq(DefaultValues::DEFAULT_STATUS_NAME)
    ...
    it 'check creating new status with correct status_data | no color' do
      color_name = 30.times.map { StaticData::ALPHABET.sample }.join
      request = Net::HTTP::Post.new('/status_new', 'Content-Type' => 'application/json')
      request.set_form_data(account.merge({"status_data[name]" => color_name}))
      response = http.request(request)
      expect(response.code).to eq('200')
      expect(JSON.parse(response.body)['errors'].empty?).to be_truthy
      expect(JSON.parse(response.body)['status']['name']).to eq(color_name)
      expect(JSON.parse(response.body)['status']['color']).to eq(DefaultValues::DEFAULT_STATUS_COLOR)
    end
  end
end