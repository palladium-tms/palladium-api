...
class RunFunctions
  ...
  def self.create_new_run(http, options = {})
    options[:run_name] ||= http.random_name
    options[:plan_name] ||= http.random_name
    params = if options[:plan_id]
               {"run_data[plan_id]": options[:plan_id]}
             elsif options[:product_id]
               {"plan_data[product_id]": options[:product_id],
                "plan_data[name]": options[:plan_name]}
             else
               {"plan_data[name]": options[:plan_name],
                "plan_data[product_name]": options[:product_name]}
             end
    responce = http.post_request('/api/run_new', params.merge("run_data[name]": options[:run_name]))
