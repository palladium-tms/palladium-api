...
class ProductFunctions
  ...
  def self.create_new_product(http, product_name = 30.times.map { StaticData::ALPHABET.sample }.join)
    response = http.post_request('/api/product_new', {"product_data[name]": product_name})
  ...
  def self.update_product(http, product_id,  product_name = 30.times.map { StaticData::ALPHABET.sample }.join)
    http.post_request('/api/product_edit',{"product_data[id]": product_id, "product_data[name]": product_name})
  end
